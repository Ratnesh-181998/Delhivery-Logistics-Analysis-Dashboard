...
# ==========================================
print("\n" + "="*50)
print("DELHIVERY ANALYSIS RESULTS")
print("="*50 + "\n")
print(f"Dataset Shape: {df.shape}")
print(f"Missing Values:\n{df.isna().sum()[df.isna().sum() > 0]}")
print("\nHYPOTHESIS TESTS:")
try:
    ks_res = stats.ks_2samp(time_taken_btwn_odstart_and_od_end["time_taken_btwn_odstart_and_od_end"], start_scan_to_end_scan["start_scan_to_end_scan"])
    print(f"1. Time Taken vs Scan Time (KS Test): {ks_res}")
except Exception as e: print(f"Test 1 failed: {e}")
try:
    ks_res2 = stats.ks_2samp(actual_time["actual_time"], start_scan_to_end_scan["start_scan_to_end_scan"])
    print(f"2. Actual Time vs Scan Time (KS Test): {ks_res2}")
except Exception as e: print(f"Test 2 failed: {e}")
try:
    ks_res3 = stats.ks_2samp(actual_time["actual_time"], osrm_time["osrm_time"])
    print(f"3. Actual Time vs OSRM Time (KS Test): {ks_res3}")
except Exception as e: print(f"Test 3 failed: {e}")
